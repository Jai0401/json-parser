...
def parse_object(tokens):
    ...
    while True:
        ...
        if tokens[0] == JSON_CLOSE_BRACE:
            raise Exception('Trailing comma in object')
...
def parse(tokens, is_root=False):
    ...
    if is_root and t != JSON_OPEN_BRACE:
        raise Exception('Root must be an object')

    if t == JSON_OPEN_BRACKET:
        return parse_array(tokens[1:])
    elif t == JSON_OPEN_BRACE:
